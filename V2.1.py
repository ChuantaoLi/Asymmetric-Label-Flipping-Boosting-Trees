...
def ada_boost_train_ds_with_flipping(data_arr, class_labels, num_it=100):
    ...
    # ==================== 核心修改：定义翻转增益因子 ====================
    # 为安全点设置一个较低的基础翻转增益，而不是完全不翻转（0）。
    # 这允许模型偶尔在“安全”区域进行探索，增加了模型的鲁棒性。
    BASELINE_FLIP_GAIN = 0.5  # 安全点的翻转增益

    # 初始化所有点的增益为基础增益
    flipping_gain = np.full_like(boundary_scores, fill_value=BASELINE_FLIP_GAIN)
    # 将边界点的增益设置为1.0
    flipping_gain[boundary_scores == 1.0] = 1.0
    # =======================================================================

    print("--- Starting AdaBoost Training with NaNs-Guided Boundary Flipping ---")
    for i in range(num_it):
        ...
        if i > 0:
            ...
            base_flipping_probs = posterior_prob[majority_indices].flatten()

            # ==================== 核心修改：应用翻转增益 ====================
            # 使用新的“翻转增益”替代原来的“边界分数”
            gain_for_majority = flipping_gain[majority_indices]

            # 调整翻转概率 = 基础概率 * 翻转增益
            # 边界点增益为1，安全点增益为 BASELINE_FLIP_GAIN (例如0.1)
            adjusted_flipping_probs = base_flipping_probs * gain_for_majority
            # =============================================================
