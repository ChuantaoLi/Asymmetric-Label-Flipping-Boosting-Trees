...
def identify_outlier_noise_with_nn(nn_obj, y_train):
    """
    使用 Natural_Neighbor 的结果来识别离群点和噪声点（适用于所有类别）。
    """
    print("--- Identifying outlier and noise samples using Natural Neighbors ---")
    is_outlier_noise = np.zeros(len(y_train), dtype=bool)
    outlier_count = 0
    for i in range(len(y_train)):
        neighbors = nn_obj.nan.get(i, set())
        ...
        neighbor_labels = y_train[list(neighbors)]
        if np.all(neighbor_labels != y_train[i]):
            is_outlier_noise[i] = True
            outlier_count += 1
...
def ada_boost_train_ds_with_flipping(data_arr, class_labels, num_it=100):
    ...
    is_outlier = identify_outlier_noise_with_nn(nn, class_labels)
    OUTLIER_SUPPRESSION_FACTOR = 0.1
    cost_factor = np.ones(m)
    cost_factor[is_outlier] = OUTLIER_SUPPRESSION_FACTOR
    ...
    # 涟漪拓展机制的准备
    EXPANSION_NEIGHBORS = 5  # 用于评估拓展点周围环境的邻居数
    MAX_PROXIMITY_BOOST = 2.0  # 最大拓展增益
    ...
    for i in range(num_it):
        ...
        if i > 0:
            ...
            # >>> 核心修改：实现基于少数类密度的智能“涟漪”拓展 <<<
            # 1. 动态邻近度增益数组，基础为1（无增益）
            proximity_gain = np.ones(m)

            # 2. 如果上一轮有翻转样本，则启动拓展机制
            if len(last_flipped_indices) > 0:
                # a. 找到上一轮翻转点的邻居，作为本轮的“拓展候选区”
                _, neighbor_indices_of_flipped = nn_model_for_expansion.kneighbors(data_arr[last_flipped_indices])
                expansion_candidate_indices = np.unique(neighbor_indices_of_flipped.flatten())

                # b. 仅对候选区中的“多数类”样本计算其拓展潜力
                majority_mask = (encoded_labels == -1)
                candidate_mask = np.zeros(m, dtype=bool)
                candidate_mask[expansion_candidate_indices] = True
                final_expansion_points_indices = np.where(majority_mask & candidate_mask)[0]

                if len(final_expansion_points_indices) > 0:
                    # c. 评估拓展点的周边环境：查找每个拓展点的邻居
                    _, neighbors_of_points = nn_model_for_expansion.kneighbors(data_arr[final_expansion_points_indices])

                    # d. 计算每个拓展点周围的“少数类邻居数量”
                    minority_neighbor_counts = np.sum(class_labels[neighbors_of_points] == minority_label, axis=1)

                    # e. 将“少数类邻居数量”映射为具体的增益值
                    #    公式: 1 + (最大增益 - 1) * (少数邻居数 / K)
                    calculated_gains = 1.0 + (MAX_PROXIMITY_BOOST - 1.0) * (minority_neighbor_counts / EXPANSION_NEIGHBORS)

                    # f. 将计算出的动态增益应用到 proximity_gain 数组中
                    proximity_gain[final_expansion_points_indices] = calculated_gains
            ...
            adjusted_flipping_probs = np.clip(base_probs * static_gain * dynamic_gain, 0, 1.0)
        ...
        if (i + 1) % 10 == 0 or (i + 1) == num_it or i == 0:
